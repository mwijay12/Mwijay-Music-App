...
const SongListItem: React.FC<{ 
    ...
    return (
         <li ref={ref} key={song.id} className={`group flex items-center gap-4 p-4 rounded-lg hover:bg-[var(--surface-color)] transition-colors opacity-0 ${isNewlyAdded ? 'newly-added-highlight' : ''}`}>
            ...
            <div className="flex-1 min-w-0" onClick={onOpenDetails}>
             ...
             <div className="flex items-center flex-shrink-0">
                <button onClick={onAddToQueue} className="control-icon w-12 h-12 rounded-full text-neutral-400 hover:text-white flex items-center justify-center transition-opacity focus:opacity-100" aria-label={`Add ${song.title} to queue`} title="Add to Queue">
                    <i className="fas fa-plus text-lg"></i>
                </button>
                <button onClick={onPlaySong} className="control-icon w-12 h-12 rounded-full bg-[var(--primary-accent)] text-black flex items-center justify-center ml-1 transition-opacity focus:opacity-100" aria-label={`Play ${song.title}`} title={`Play ${song.title}`}>
