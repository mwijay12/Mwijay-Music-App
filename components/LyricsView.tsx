...
const LyricsView: React.FC<LyricsViewProps> = ({ song, profile, onClose, onMinimize, onUpdateSong, onUpdateProfile, progress, duration, showNotification }) => {
    ...
    const ai = useMemo(() => {
        if (process.env.API_KEY) {
            try {
                ...
                console.error("Failed to initialize GoogleGenAI for lyrics", String(e));
                return null;
            }
        }
        return null;
    ...
    const currentLineIndex = useMemo(() => {
        if (!duration || lyricsSettings.animation !== 'karaoke' || lines.length === 0) return -1;
        const progressRatio = progress / duration;
        return Math.min(lines.length - 1, Math.floor(progressRatio * lines.length));
    }, [progress, duration, lines, lyricsSettings.animation]);
    ...
    const handleFetchLyrics = async () => {
        ...
        try {
            const response = await ai.models.generateContent({
                ...
                contents: `Find the lyrics for the song "${song.title}" by "${song.artist}".`,
                config: {
                    systemInstruction: "You are a lyric-finding expert. Your sole purpose is to return the full, accurate lyrics for a given song. Do not include any commentary, introductory text like 'Here are the lyrics', or formatting like artist/title headers. Only return the raw lyrics text itself, formatted with line breaks.",
                }
            });
            const fetchedLyrics = response.text.trim();
            setLyricsText(fetchedLyrics);
            onUpdateSong({ ...song, lyrics: fetchedLyrics });
            showNotification('Lyrics found!', 'success');
        } catch (error) {
            console.error("Error fetching lyrics:", String(error));
            showNotification('Could not find lyrics for this song.', 'error');
        } finally {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 z-[70] flex items-center justify-center p-4" onClick={onClose}>
            <div 
                className="glass-modal-card rounded-3xl flex flex-col w-full max-w-md h-[80vh] max-h-[600px] overflow-hidden shadow-2xl relative" 
                onClick={e => e.stopPropagation()}
            >
                <div className="lyrics-view-bg" style={{ backgroundImage: `url(${song.albumArtUrl})` }} />
                ...
                <div className="relative z-10 flex flex-col h-full">
                            ...
                            >
                                {lines.map((line, index) => {
                                    ...
                                    return (
                                        <p
                                            ...
                                            className={`lyrics-line ${lyricsSettings.animation} ${isActive ? 'active' : ''}`}
                                ...
                                {!lyricsText && (
                                    <div className="text-center text-neutral-400">
                                        ...
                                        <button onClick={handleFetchLyrics} disabled={isFetchingLyrics} className="mt-4 bg-[var(--primary-accent)] text-black font-bold py-2 px-4 rounded-full flex items-center justify-center gap-2">
                                            {isFetchingLyrics ? <i className="fas fa-spinner fa-spin"></i> : <><i className="fas fa-search mr-2"></i> Find with AI</>}
                                        </button>
                                    </div>
                                )}
                            </div>
                    ...
                    <footer className="p-4 flex-shrink-0 glass-modal-card !border-t !border-white/10">
                        {isEditing ? (
                        ...
                        <div className="absolute inset-x-4 bottom-24 bg-black/80 backdrop-blur-md p-4 rounded-xl z-10 space-y-4">
                            <div>
                                ...
                                <div className="grid grid-cols-4 gap-2 mt-1">
                                    <button onClick={() => handleSettingChange('animation', 'karaoke')} className={`p-2 text-xs rounded ${lyricsSettings.animation === 'karaoke' ? 'bg-[var(--primary-accent)] text-black' : 'bg-white/10'}`}>Karaoke</button>
