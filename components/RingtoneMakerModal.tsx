        ...
        if (song.audioData) {
            ...
            audioContextRef.current.decodeAudioData(song.audioData.slice(0))
                .then(buffer => {
                    ...
                    setIsLoading(false);
                    // Set initial end time to a reasonable max
                    setEndTime(Math.min(30, buffer.duration));
                })
                .catch(err => {
                    console.error("Error decoding audio data:", String(err));
                    showNotification("Could not load audio for trimming.", 'error');
                    onClose();
                });
        }
        return () => {
            audioContextRef.current?.close().catch(e => console.error("Error closing AudioContext:", String(e)));
        }
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[80] flex items-center justify-center p-4" onClick={onClose}>
            <div className="liquid-glass-pane glare-effect rounded-2xl flex flex-col w-full max-w-lg max-h-[90vh] overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                {isLoading ? (
                    <div className="flex-1 flex items-center justify-center"><Loader2 className="animate-spin text-2xl" /></div>
                ) : (
                ...
                <footer className="p-4 border-t border-white/10 flex-shrink-0 flex items-center justify-between">
                     <button onClick={isPlayingPreview ? handleStopPreview : handlePlayPreview} className="bg-white/10 font-bold py-2 px-5 rounded-full flex items-center gap-2" disabled={isLoading}>
                        {isPlayingPreview ? <Square size={18} /> : <Play size={18} />}
                        {isPlayingPreview ? 'Stop' : 'Preview'}
                    </button>
                    <button onClick={handleDownload} className="bg-[var(--primary-accent)] text-black font-bold py-2 px-5 rounded-full flex items-center gap-2" disabled={isLoading}>
                         <Download size={18} />
                        Download .wav
                    </button>
                </footer>
