...
const commandAliases: Record<string, string> = {
    ...
    "cancel timer": "cancel sleep timer",
    "stop timer": "cancel sleep timer",
    "disable timer": "cancel sleep timer",
};

// FIX: Hoisted this function above the `questions` object to prevent a ReferenceError.
...
const questions: Record<string, ResponseGenerator> = {
    ...
    "cancel sleep timer": ({ controls }) => {
        controls.setSleepTimer('off', 0);
    ...
    "search online": () => 'Online search requires an internet connection. Please switch to Online Mode.',
    ...
    "help": () => {
        return "You can ask me to play music, change themes, or tell you what's playing. Try saying 'play some radio' or 'switch to light theme'. For more, check the Help section in your Profile.";
    },
};
...
const findCommandMatch = (command: string): string | null => {
    ...
    if (questions[normalizedCommand]) {
    ...
    const mainKeys = Object.keys(questions);
...
export const assistantKnowledge: {
    ...
    processCommand: async (text: string, context: CommandContext): Promise<string> => {
        ...
        if (matchedCommand && questions[matchedCommand]) {
            try {
                const response = await Promise.resolve(questions[matchedCommand](context));
                return response;
            } catch (error) {
